...
export async function PATCH(request) {
  const { id, color, qty } = await request.json();
  ...
  try {
    ...
    if (!id || !color || isNaN(quantityToDecrease)) {
      return NextResponse.json(
        { error: "Missing or invalid id, color, or quantity" },
    ...
    if (!product) {
      return NextResponse.json(
        { error: "Product with specified id and color not found" },
...
export async function GET(request, { params }) {
  try {
    ...
    const product = await collection.findOne(
      { _id: new ObjectId(id), "color.color": color },
      { projection: { color: 1 } }
    );

    if (!product) {
      return NextResponse.json(
        { error: "Product or color not found" },
        { status: 404, headers: corsHeaders }
      );
    }
    ...
    return NextResponse.json({ qty: colorEntry.qty }, { status: 200, headers: corsHeaders });
